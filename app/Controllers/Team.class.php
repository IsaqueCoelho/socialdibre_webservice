...
namespace App\Controllers{
    ...
    class Team{
        ...
        public function getAll($page){
            ...
            $sql = "SELECT tm.id_team, tm.tm_name, tm.tm_image, tm.tm_points, (SELECT COUNT(DISTINCT tm_points) FROM team WHERE tm_points >= tm.tm_points) AS tm_position, (SELECT COUNT(ppt.id_user) FROM `participates` AS ppt WHERE ppt.id_team = tm.id_team) AS usr_players FROM `team` AS tm ORDER BY tm.tm_points DESC LIMIT " . $final_page . ", 10";
            ...
            if($result > 0){
                ...
                $return_data = ["success" => 1, "page" => $page, "result" => $list_teams, "message" => "Ranking de Times!"];
            
            } else {

                $return_data = ["success" => 0, "page" => $page, "result" => "", "message" => "Nenhum Time encontrado!"];
        ...
        public function get($id){
            ...
            $sql = "SELECT tm.tm_name, tm.tm_image, tm.tm_points, (SELECT COUNT(DISTINCT tm_points) FROM `team` WHERE tm_points >= tm.tm_points) As tm_position, (SELECT COUNT(ppt.id_user) FROM `participates` AS ppt WHERE ppt.id_team = tm.id_team) AS usr_players, (SELECT COUNT(clg.id_challenge) FROM `challenge` AS clg WHERE (clg.id_challenger = tm.id_team OR clg.id_challenged = tm.id_team) AND clg.clg_status = '1' ) AS tm_challenges FROM `team` AS tm WHERE tm.id_team = ?";
            ...
            if($result > 0){
                ...
                $user_data = [
                    "name" => $result['tm_name'],
                    "image" => $result['tm_image'],
                    "points" => $result['tm_points'],
                    "position" => $result['tm_position'],
                    "players" => $result['usr_players'],
                    "challenges" => $result['tm_challenges']
                ];
    
                $return_data = ["success" => 1, "result" => $user_data, "message" => "Time encontrado!"];

            } else {

                $return_data = ["success" => 0, "result" => "", "message" => "Time não encontrado!"];
